...
def write_joint(index, indent, bvh_header):
    ...
    if joint_name in ["PELVIS", "HIP_RIGHT", "HIP_LEFT"]:
        bvh_header += f"{'    ' * (indent + 1)}CHANNELS 6 Xposition Yposition Zposition Xrotation Yrotation Zrotation\n"
    else:
...
with open(os.path.join(folder_path, file_name), 'r') as file:
    ...
    for line in joint_data:
        joint_name = extract_joint_name(line)
        if joint_name == "HIP_LEFT":
            hip_left_init_world_pos = extract_joint_world_pos(line)
        elif joint_name == "HIP_RIGHT":
            hip_right_init_world_pos = extract_joint_world_pos(line)
    ...
    with open(os.path.join(folder_path, file_name), 'r') as file:
        ...
        for line in joint_data:
            ...
            if joint_name == "PELVIS":
                pelvis_world_pos = extract_joint_world_pos(line) - pelvis_init_world_pos
                pelvis_world_pos *= 0.1  # TODO 임시
                bvh_motion += f"{pelvis_world_pos[0]} {pelvis_world_pos[1]} {pelvis_world_pos[2]} "
